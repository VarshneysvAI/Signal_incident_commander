import { useState } from 'react';

import { useAppStore } from '../store';
import { queryApi } from '../api/client';

export function QueryBar() {
  const [query, setQuery] = useState('');
  const [loading, setLoading] = useState(false);
  const currentIncident = useAppStore((state) => state.currentIncident);
  const setLastQueryResult = useAppStore((state) => state.setLastQueryResult);
  const lastQueryResult = useAppStore((state) => state.lastQueryResult);

  const handleQuery = async () => {
    if (!currentIncident || !query.trim()) return;
    
    setLoading(true);
    try {
      const response = await queryApi.query(currentIncident.id, {
        speaker_name: 'User',
        text: query,
      });
      
      setLastQueryResult({
        question: query,
        answer: response.data.answer,
        sources: response.data.grounded_node_ids || [],
      });
      setQuery('');
    } catch (error) {
      console.error('Failed to query:', error);
    } finally {
      setLoading(false);
    }
  };

  return (
    <div className="bg-slate-800 border-t border-slate-700 p-4">
      <div className="flex gap-2 mb-3">
        <input
          type="text"
          placeholder="Ask SIGNAL... (e.g., 'Signal, what is our status?')"
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          onKeyPress={(e) => e.key === 'Enter' && handleQuery()}
          disabled={!currentIncident || loading}
          className="flex-1 bg-slate-700 text-white px-4 py-2 rounded border border-slate-600 focus:outline-none focus:border-blue-500 disabled:opacity-50"
        />
        <button
          onClick={handleQuery}
          disabled={!currentIncident || loading || !query.trim()}
          className="bg-blue-600 hover:bg-blue-700 disabled:bg-slate-600 text-white px-6 py-2 rounded font-medium transition-colors"
        >
          {loading ? 'Asking...' : 'Ask'}
        </button>
      </div>

      {lastQueryResult && (
        <div className="bg-slate-900 rounded-lg p-4 border border-slate-700">
          <div className="flex items-start gap-3">
            <div className="w-8 h-8 bg-blue-600 rounded-full flex items-center justify-center flex-shrink-0">
              <span className="text-white text-sm font-bold">S</span>
            </div>
            <div className="flex-1">
              <p className="text-slate-400 text-xs mb-1">{lastQueryResult.question}</p>
              <p className="text-white text-sm">{lastQueryResult.answer}</p>
              {lastQueryResult.sources && lastQueryResult.sources.length > 0 && (
                <p className="text-slate-500 text-xs mt-2">
                  Sources: {lastQueryResult.sources.length} node(s)
                </p>
              )}
            </div>
          </div>
        </div>
      )}
    </div>
  );
}
